...
int SeviyeSayac(int MainListUzunluk)
{ // seviye sayma fonksiyonu
    int SeviyeSayaci = 0;
...
void LinkedListYazdir(struct LinkedList **HeadlerDizisi, int SeviyeSayaci)
{ // rekursiv yazdirmada olmadigi icin seviye sayaci tutarak yazdirma fonksiyonu olusturdum
    int i;
...
void FreeNodes(struct LinkedList *MainHead)
{ // nodelari freeleme fonksiyonu calisma prensipi 2 pointer ile butun nodelari gezip freeler
...
int Randomized(int RecursiveCallNumber, int MainListUzunluk)
{ // random olusturma fonksiyonu
...
{
    if (MainListUzunluk == 0 || MainListUzunluk == 1)
    { // construct etme fonksiyonu rekursif sekilde olusturur
...
void ElemanArama(struct LinkedList **HeadlerDizisi, int SeviyeSayaci)
{
    ...
    for (i = SeviyeSayaci - 1; i >= 0; i--)
    {
        ...
        while (tmp != NULL && tmp->date != aranan && Exit_while == 0 /*&& tmp->NextLevel != NULL*/)
        {
            ...
            { // aradigim deger suanki degerimden buyuk ise bu seviyeden aramayi durdurur ve diger seviyeden aramaya baslar.
                Exit_while = 1;
            }
            else if (tmp->date == aranan)
            {
...
{ // main listte elemani ekler sonra construct eder.
    ...
    if (*head_ref == NULL || (*head_ref)->date >= new_node->date)
    {
    ...
    {
        current = *head_ref;
        while (current->Next != NULL && current->Next->date < new_node->date)
        {
